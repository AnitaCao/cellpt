...
from config.opts import parse_args, add_lora_args, add_multiview_args
...
from utils.utils import (
    ...
    worker_init_fn, find_group_lr, apply_slide_mask, log_to_wandb, MacroF1EarlyStopping,
...
# -----------------------------
# Multi-view scale gate
# -----------------------------
class ScaleGate(nn.Module):
    def __init__(self, d_embed: int, n_scales: int, hidden: int | None = None, entropy_w: float = 0.1):
        super().__init__()
        h = hidden or max(128, d_embed // 2)
        self.n = n_scales
        self.entropy_w0 = float(entropy_w)
...
# -----------------------------
class CoarseHeadMLP(nn.Module):
    """
    Small MLP head for coarse supervision on pre_logits (after fusion).
    Enabled by flag: --coarse_head_mlp 1
    """
    def __init__(self, d: int, g: int, hidden: int | None = None, dropout: float = 0.0):
        super().__init__()
        h = hidden or max(d, 256)
        self.net = nn.Sequential(
            nn.LayerNorm(d),
            nn.Linear(d, h),
            nn.GELU(),
...
def build_backbone_and_heads(args, classes, num_classes, device):
    ...
    with torch.no_grad():
        ...
        _out = model(_probe)
        assert _out.shape[-1] == len(classes), "Cosine head replacement failed"
        feats = model.forward_features(_probe)
        pre_logits = model.forward_head(feats, pre_logits=True)
    d_embed = int(pre_logits.shape[-1])
...
def eval_epoch_generic(model, val_loader, device, amp_dtype, adj, args,
                       use_slide_mask, allowed_bitmask, classes,
                       epoch, fine_to_coarse_idx, coarse_classes):
    model.eval()
    val_total = 0
    ...
    with torch.inference_mode(), torch.amp.autocast('cuda', dtype=amp_dtype, enabled=torch.cuda.is_available()):
        for batch in val_loader:
            ...
            val_loss_adj_sum += loss.item() * y.size(0)
            c, t_ = compute_metrics(logits_eff, y)
            val_correct_adj += c; val_total += t_
            all_preds.append(logits_eff.argmax(1)); all_targets.append(y)
    preds = torch.cat(all_preds).cpu() if all_preds else torch.empty(0, dtype=torch.long)
    targs = torch.cat(all_targets).cpu() if all_targets else torch.empty(0, dtype=torch.long)
    return {
...
def main():
    ...
    model, mean, std, lora_params, norms = build_backbone_and_heads(
        args, classes, num_classes, device
    )

    # ---- Load from checkpoint if provided ----
    if getattr(args, "initial_checkpoint", None):
        print(f"Loading initial weights from {args.initial_checkpoint}")
        ckpt = torch.load(args.initial_checkpoint, map_location=device)
        state_dict = ckpt["model"] if "model" in ckpt else ckpt
        
        # Filter out head keys if shapes mismatch (e.g. finetuning on new classes)
        ...
        print(f"Loaded encoded weights. Missing keys: {len(msg.missing_keys)}. Unexpected keys: {len(msg.unexpected_keys)}")
        if skipped:
            print(f"Skipped {len(skipped)} keys due to shape mismatch (likely head): {skipped[:5]}...")
    ...
    # Disable mixup when multi-view to keep cross-view alignment
    if int(getattr(args, "multi_view", 0)) == 1 and mixup_enabled:
        print("[multi-view] Disabling mixup/cutmix to preserve view alignment.")
        mixup_enabled, mixup_fn, soft_ce = False, None, None
    ...
    # ---- Datasets ----
    train_ds, val_ds = build_train_val_datasets(args, class_to_idx, mean, std, label_col)
    ...
    if int(getattr(args, "multi_view", 0)) == 1 and hasattr(model, "scale_gate"):
        head_weight_params += list(model.scale_gate.parameters())
    ...
    if use_coarse_aux:
        with torch.inference_mode():
            ...
            feats_p = model.forward_features(_probe)
            pre_p   = model.forward_head(feats_p, pre_logits=True)
        d_embed = int(pre_p.shape[-1])
        if int(getattr(args, "coarse_head_mlp", 0)) == 1:
            hidden  = int(getattr(args, "coarse_mlp_hidden", 0)) or None
            dropout = float(getattr(args, "coarse_mlp_dropout", 0.0))
            model.coarse_head = CoarseHeadMLP(d_embed, len(coarse_classes),
                                              hidden=hidden, dropout=dropout).to(device)
            print(f"[coarse] Using MLP head: D={d_embed}, hidden={hidden or max(d_embed, 256)}, "
                  f"dropout={dropout}, G={len(coarse_classes)}")
        else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if ema_model is not None and ema_eval:
            ema_metrics = eval_epoch_generic(
                ...
                classes=classes,
                epoch=epoch,
                fine_to_coarse_idx=fine_to_coarse_idx,
                coarse_classes=coarse_classes,
            )
            # If you want EMA macro-F1 in logs:
            if ema_metrics.get("preds") is not None and ema_metrics["preds"].numel() > 0:
                _pc, _, _ = compute_per_class_metrics(ema_metrics["preds"], ema_metrics["targets"], classes)
                ema_metrics["macro_f1"] = float(_pc.get("macro_f1", 0.0))
