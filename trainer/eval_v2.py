#!/usr/bin/env python3
"""
eval.py - Evaluate CellPT/MorphPT checkpoints

Fixes applied:
- Gate analysis moved after tensor concatenation
- JSON serialization handles numpy types
- Import moved outside loop
- Predictions saved with original CSV data for error analysis
"""
...
from utils.utils import (
    ...
    apply_slide_mask,  # Moved import here (was inside loop)
)
...
def build_eval_dataset(args_ns, class_to_idx, mean, std, label_col):
    ...
    Returns (dataset, original_dataframe) for later merging with predictions.
    ...
    if mv:
        ...
        # Keep raw_img_path from 2.5x as primary, rename 10x version
        key_col = "cell_id"
        
        # Select columns from view A (2.5x) - keep raw_img_path as-is
        cols_a = [c for c in df_a.columns]
        
        # Select only key_col and raw_img_path from view B (10x), rename the path
        df_b_subset = df_b[[key_col, 'raw_img_path']].copy()
        df_b_subset = df_b_subset.rename(columns={'raw_img_path': 'raw_img_path_10x'})
        
        # Merge to get both paths
        original_df = df_a.merge(df_b_subset, on=key_col, how='inner')
        
        # Filter to only cells that are in the class_to_idx
        ...
        # Load original CSV to preserve all columns
        original_df = pd.read_csv(csv_single)
        original_df = original_df[original_df[label_col].isin(class_to_idx.keys())].reset_index(drop=True)
    
    # Filter dataset rows to known classes
    before = len(ds.df)
    ...
    print(f"[info] Original CSV columns preserved: {original_df.columns.tolist()}")
...
def main():
    ...
    ds, original_df = build_eval_dataset(eval_args, class_to_idx, mean, std, label_col)
    ...
    print(f"\n[coarse] macro_recall={coarse_metrics['coarse_macro_recall']:.4f}  "
          f"overall_acc={coarse_metrics['coarse_overall_acc']:.4f}")

    # ================================================================
    # GATE ANALYSIS (now after concatenation - fixed!)
    # ================================================================
    ...
    # Merge predictions with original CSV data
    ...
    results_df["confidence"] = all_confidences.numpy()
    
    # Add probability for predicted and true class
    ...
    print(f"Saved predictions with original data → {out_dir / 'predictions_full.csv'}")
    
    # Save misclassified samples for easy error analysis
    misclassified_df = results_df[~results_df["correct"]].copy()
    misclassified_df = misclassified_df.sort_values("confidence", ascending=False)  # High confidence errors first
    misclassified_df.to_csv(out_dir / "misclassified.csv", index=False)
    print(f"Saved misclassified samples → {out_dir / 'misclassified.csv'} ({len(misclassified_df)} samples)")
    
    # Save high-confidence errors (most interesting for debugging)
    ...
    if len(high_conf_errors) > 0:
        ...
        print(f"Saved high-confidence errors → {out_dir / 'high_confidence_errors.csv'} ({len(high_conf_errors)} samples)")
    ...
    print(f"Saved metrics summary → {out_dir / 'metrics_eval.json'}")

    # Save per-class metrics (with proper serialization)
    pc_clean = json_serializable(pc)
    with open(out_dir / "per_class_metrics.json", "w") as f:
        json.dump(pc_clean, f, indent=2)
    print(f"Saved per-class metrics → {out_dir / 'per_class_metrics.json'}")
